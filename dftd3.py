...
## Published parameters for zero-damping optimized for different functionals
...
class calcD3:
   def __init__(self, file, s6, rs6, s8, damp, abc, intermolecular):
      ...
      for j in range(0,natom):
         ...
         for k in range(j+1,natom):
            ...
            if intermolecular == "on":
               if mols[j] == mols[k]:
                  scalefactor = 0
                  print "   --- ignoring interactions between atoms",(j+1), "and", (k+1)
...
if __name__ == "__main__":
   ...
   files = []
   damp = "zero"; zero_s6 = 0.0; zero_rs6 = 0.0; zero_s8 = 0.0; abc_term = "off"; intermolecular = "off"
   if len(sys.argv) > 1:
      for i in range(1,len(sys.argv)):
         if sys.argv[i] == "-damp": damp = (sys.argv[i+1])
         elif sys.argv[i] == "-s6": zero_s6 = float(sys.argv[i+1])
         elif sys.argv[i] == "-rs6": zero_rs6 = float(sys.argv[i+1])
         elif sys.argv[i] == "-s8": zero_s8 = float(sys.argv[i+1])
         elif sys.argv[i] == "-3body": abc_term = (sys.argv[i+1])
         elif sys.argv[i] == "-intermolecular": intermolecular = (sys.argv[i+1])
         else:
   ...
   else: print "\nWrong number of arguments used. Correct format: dftd3.py (-damp zero/bj) (-s6 val) (-rs6 val) (-s8 val) (-intermolecular on/off) file(s)\n"; sys.exit()

   for file in files:
      fileD3 = calcD3(file, zero_s6, zero_rs6, zero_s8, damp, abc_term, intermolecular)
